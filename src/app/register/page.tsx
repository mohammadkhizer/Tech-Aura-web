...
const NON_TECH_EVENTS = [
  "Maths Quiz",
  "Debate",
  "eSports Tournament (BGMI/Free Fire)",
  "Treasure Hunt"
];
...
export default function RegisterPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Simulate static network delay for registration
    setTimeout(() => {
      ...
      <div className="pt-48 pb-20 px-6 flex flex-col items-center justify-center text-center stagger-reveal">
        ...
        <div className="max-w-md space-y-6">
          <p className="text-muted-foreground text-lg uppercase tracking-widest font-light">
            Identity Verified. Welcome to the Aura.
          </p>
    ...
    <div className="pt-32 pb-40 px-6 max-w-5xl mx-auto min-h-screen">
      <div className="text-center mb-16 stagger-reveal">
        ...
        <h1 className="font-headline text-5xl md:text-7xl mb-4 tracking-tighter uppercase text-white leading-none">
          JOIN THE <span className="text-primary text-glow">AURA</span>
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start stagger-reveal">
        <div className="lg:col-span-4 space-y-6">
          <div className="glass-panel p-8 border-primary/20 rounded-none bg-black/40">
            ...
            <ul className="space-y-4 text-[10px] text-muted-foreground uppercase tracking-widest leading-loose">
              ...
              <li className="flex gap-3"><span className="text-primary">03</span> Static demo mode active.</li>
            </ul>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-8 glass-panel p-8 md:p-12 space-y-8 rounded-none border-primary/10 shadow-2xl relative overflow-hidden bg-black/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="email" name="email" type="email" placeholder="USER@AURA.TECH" required className="bg-white/5 border-white/10 rounded-none h-12 pl-10 focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm tracking-widest uppercase" />
                </div>
              </div>
              <div className="space-y-2">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-[10px] tracking-widest uppercase text-muted-foreground ml-1">Event Track</Label>
              <Select name="selectedEvent" required>
                <SelectTrigger className="bg-white/5 border-white/10 rounded-none h-14 focus:border-primary focus:ring-primary uppercase tracking-widest text-[11px] text-white">
                  <SelectValue placeholder="INITIALIZE EVENT SELECTION" />
                </SelectTrigger>
                <SelectContent className="glass-panel border-primary/30 text-white rounded-none bg-black/95 backdrop-blur-xl">
                  <SelectGroup>
                    <SelectLabel className="text-[10px] text-primary tracking-widest px-4 py-2 border-b border-white/5">TECHNICAL TRACK</SelectLabel>
                    {TECH_EVENTS.map(event => (
                      <SelectItem key={event} value={event} className="text-[10px] tracking-widest uppercase py-3 cursor-pointer">
                        {event}
                      </SelectItem>
                    ))}
                  </SelectGroup>
                  <SelectGroup>
                    <SelectLabel className="text-[10px] text-accent tracking-widest px-4 py-2 border-b border-white/5 mt-2">NON-TECHNICAL TRACK</SelectLabel>
          ...
          <div className="flex items-center justify-center gap-4 pt-4 border-t border-white/5">
            ...
            <p className="text-[8px] text-muted-foreground uppercase tracking-[0.3em] font-medium whitespace-nowrap">
              Secure Static Protocol
            </p>
            <div className="h-[1px] flex-1 bg-white/5" />
          </div>
        </form>
